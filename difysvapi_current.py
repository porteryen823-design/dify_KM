    ...
    async def _workflow_messages(
        ...
        async for chunk in self.dify_client.workflow_run(
            ...
            if chunk['event'] == 'node_started':
                ...
                # Dynamically pick first non-empty string output (instead of hardcoded 'summary')
                outputs = chunk.get('data', {}).get('outputs', {})
                workflow_contents = ''
                for val in outputs.values():
                    if isinstance(val, str) and val.strip():
                        workflow_contents = val
                        break
                content, _ = self._process_thinking_content(workflow_contents)
    ...
    async def _chat_messages_chunk(
        ...
        async for chunk in self.dify_client.chat_messages(
            ...
            if chunk['event'] == 'message_end':
                ...
                # Antigravity Fix: Map outputs to contents
                outputs = chunk.get('data', {}).get('outputs', {})
                for val in outputs.values():
                    if isinstance(val, str) and val.strip():
                        workflow_contents = val
    ...
    async def _workflow_messages_chunk(
        ...
        async for chunk in self.dify_client.workflow_run(
            ...
            if chunk['event'] == 'text_chunk':
                ...
                if remove_think:
                    # Apply a robust regex to strip <think>...</think> on the fly when emitting
                    # We will output workflow_contents after stripping it below
                    pass
            ...
            if messsage_idx % 8 == 0 or is_final:
                out_content = workflow_contents
                if remove_think:
                    out_content, _ = self._process_thinking_content(out_content)
                yield provider_message.MessageChunk(
                    role='assistant',
                    content=out_content,
                    is_final=is_final,
