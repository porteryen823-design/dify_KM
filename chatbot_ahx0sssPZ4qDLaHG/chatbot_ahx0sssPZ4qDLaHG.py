...
# --- 設定區 ---
API_KEY = "app-GdVjmTAUl173fDaSSH5Nl2iP"
BASE_URL = "http://localhost:8080/v1/chat-messages"
INPUT_FILE = "chatbot_ahx0sssPZ4qDLaHG_questions.xlsx"
...
if __name__ == "__main__":
    ...
    print(f"開始進行自動化測試 (總共 {total_q} 題)...")
    
    for index, row in df_questions.iterrows():
        no = row['NO']
        question = row['問題']
        
        print(f"[{index+1}/{total_q}] 正在處理第 {no} 題: {question[:30]}...", end=" ", flush=True)
