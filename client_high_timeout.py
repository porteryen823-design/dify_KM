...
class AsyncDifyServiceClient:
    ...
    async def workflow_run(
        ...
        print(f"DEBUG: Request to: {self.base_url}/workflows/run with timeout: {timeout}")
        async with httpx.AsyncClient(
            base_url=self.base_url,
            trust_env=True,
            timeout=timeout,
    ...
    async def upload_file(
        ...
        if isinstance(file, Path):
            ...
            file = file.read()
        async with httpx.AsyncClient(base_url=self.base_url, trust_env=True, timeout=timeout) as client:
